...
func GetEthBalance(ethAddr string, cb models.GetBalanceCallback) error {
	go func() {
		...
		if err != nil {
			...
			cb.OnBalanceAvailable(zcncore.StatusError, 0, err.Error())
			return
		}
		cb.OnBalanceAvailable(zcncore.StatusSuccess, value, "")
	}()
	return nil
}
...
func getBalanceFromEthNode(ethAddr string) (int64, error) {
	if client, err := GetEthClient(); err == nil {
		...
		zcncore.Logger.Info("balance", balance.String())
