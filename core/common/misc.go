...
func (wp WhoPays) Validate() (err error) {
	...
	return zchainErrors.New("validate_error", fmt.Sprintf("unknown WhoPays value: %d", int(wp)))
}
...
func (wp *WhoPays) Parse(val string) (err error) {
	...
	case "3rd_party":
		(*wp) = WhoPays3rdParty
	default:
		err = zchainErrors.New("parse_error", fmt.Sprintf("empty or unknown 'who_pays' value: %q", val))
	}
	return
}
