...
func (b0 *BLS0ChainScheme) RecoverKeys(mnemonic string) (*Wallet, error) {
	if mnemonic == "" {
		return nil, zchainErrors.New("recover_keys", "Set mnemonic key failed")
	}
	if b0.PublicKey != "" || b0.PrivateKey != "" {
		return nil, zchainErrors.New("recover_keys", "Cannot recover when there are keys")
	}
	b0.Mnemonic = mnemonic
...
func (b0 *BLS0ChainScheme) SetPrivateKey(privateKey string) error {
	if b0.PublicKey != "" {
		return zchainErrors.New("set_private_key", "cannot set private key when there is a public key")
	}
	if b0.PrivateKey != "" {
		return zchainErrors.New("set_private_key", "private key already exists")
	}
...
func (b0 *BLS0ChainScheme) SetPublicKey(publicKey string) error {
	if b0.PrivateKey != "" {
		return zchainErrors.New("set_public_key", "cannot set public key when there is a private key")
	}
	if b0.PublicKey != "" {
		return zchainErrors.New("set_public_key", "public key already exists")
	}
...
func (b0 *BLS0ChainScheme) rawSign(hash string) (*bls.Sign, error) {
	var sk bls.SecretKey
	if b0.PrivateKey == "" {
		return nil, zchainErrors.New("raw_sign", "private key does not exists for signing")
	}
	...
	if rawHash == nil {
		return nil, zchainErrors.New("raw_sign", "failed hash while signing")
	}
	sk.SetByCSPRNG()
...
func (b0 *BLS0ChainScheme) Verify(signature, msg string) (bool, error) {
	if b0.PublicKey == "" {
		return false, zchainErrors.New("verify", "public key does not exists for verification")
	}
	var sig bls.Sign
	...
	if rawHash == nil {
		return false, zchainErrors.New("verify", "failed hash while signing")
	}
...
func (b0 *BLS0ChainScheme) GetPrivateKeyAsByteArray() ([]byte, error) {
	if len(b0.PrivateKey) == 0 {
		return nil, zchainErrors.New("get_private_key_as_byte_array", "cannot convert empty private key to byte array")
	}
...
func BLS0GenerateThresholdKeyShares(t, n int, originalKey SignatureScheme) ([]BLS0ChainThresholdScheme, error) {
	...
	if !ok {
		return nil, zchainErrors.New("bls0_generate_threshold_key_shares", "Invalid encryption scheme")
	}
...
func (b0 *BLS0ChainScheme) SplitKeys(numSplits int) (*Wallet, error) {
	if b0.PrivateKey == "" {
		return nil, zchainErrors.New("split_keys", "primary private key not found")
	}
