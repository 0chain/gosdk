...
func (ed *ED255190chainScheme) GenerateKeys() (*Wallet, error) {
	...
	if len(ed.mnemonic) == 0 {
		...
		if err != nil {
			return nil, zchainErrors.New("generate_keys", "Getting entropy failed")
		}
		ed.mnemonic, err = bip39.NewMnemonic(entropy)
		if err != nil {
			return nil, zchainErrors.New("generate_keys", "Getting mnemonic failed")
		}
	}
...
func (ed *ED255190chainScheme) GenerateKeysWithEth(mnemonic, password string) (*Wallet, error) {
	return nil, zchainErrors.New("Not supported for this scheme")
}

func (ed *ED255190chainScheme) RecoverKeys(mnemonic string) (*Wallet, error) {
	if mnemonic == "" {
		return nil, zchainErrors.New("chain_scheme_recover_keys", "Set mnemonic key failed")
	}
	if len(ed.privateKey) > 0 || len(ed.publicKey) > 0 {
		return nil, zchainErrors.New("chain_scheme_recover_keys", "Cannot recover when there are keys")
	}
...
func (ed *ED255190chainScheme) SetPublicKey(publicKey string) error {
	...
	if len(ed.publicKey) > 0 {
		zchainErrors.New("set_public_key", "public key already exists")
	}
	var err error
...
func (ed *ED255190chainScheme) Sign(hash string) (string, error) {
	if len(ed.privateKey) == 0 {
		return "", zchainErrors.New("chain_scheme_sign", "private key does not exists for signing")
	}
	...
	if rawHash == nil {
		return "", zchainErrors.New("chain_scheme_sign", "Failed hash while signing")
	}
...
func (ed *ED255190chainScheme) Verify(signature, msg string) (bool, error) {
	if len(ed.publicKey) == 0 {
		return false, zchainErrors.New("chain_scheme_verify", "public key does not exists for verification")
	}
...
func (ed *ED255190chainScheme) Add(signature, msg string) (string, error) {
	return "", zchainErrors.New("chain_scheme_add", "Not supported by signature scheme")
}
