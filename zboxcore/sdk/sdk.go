...
var sdkNotInitialized = zchainErrors.New("sdk_not_initialized", "SDK is not initialised")
...
func GetReadPoolInfo(clientID string) (info *AllocationPoolStats, err error) {
	...
	if len(b) == 0 {
		return nil, zchainErrors.New("empty response")
	}
...
func GetStorageSCConfig() (conf *StorageSCConfig, err error) {
	...
	if conf.ReadPool == nil || conf.WritePool == nil || conf.StakePool == nil {
		return nil, zchainErrors.New("invalid confg: missing read/write/stake pool configs")
	}
	return
}
...
func GetBlobber(blobberID string) (blob *Blobber, err error) {
	...
	if len(b) == 0 {
		return nil, zchainErrors.New("empty response from sharders")
	}
	blob = new(Blobber)
...
func GetAllocation(allocationID string) (*Allocation, error) {
	...
	if err != nil {
		return nil, zchainErrors.New("allocation_fetch_error", "Error fetching the allocation."+err.Error())
	}
	allocationObj := &Allocation{}
	err = json.Unmarshal(allocationBytes, allocationObj)
	if err != nil {
		return nil, zchainErrors.New("allocation_decode_error", "Error decoding the allocation."+err.Error())
	}
...
func GetAllocationsForClient(clientID string) ([]*Allocation, error) {
	...
	if err != nil {
		return nil, zchainErrors.New("allocations_fetch_error", "Error fetching the allocations."+err.Error())
	}
	allocations := make([]*Allocation, 0)
	err = json.Unmarshal(allocationsBytes, &allocations)
	if err != nil {
		return nil, zchainErrors.New("allocations_decode_error", "Error decoding the allocations."+err.Error())
	}
...
func smartContractTxnValueFee(sn transaction.SmartContractTxnData,
	...
	if t == nil {
		return "", "", zchainErrors.New("transaction_validation_failed",
...
func CommitToFabric(metaTxnData, fabricConfigJSON string) (string, error) {
	...
	if err != nil {
		return "", zchainErrors.New("fabric_config_decode_error", "Unable to decode fabric config json")
	}
	...
	if err != nil {
		return "", zchainErrors.New("fabric_config_encode_error", "Unable to encode fabric config body")
	}

	req, ctx, cncl, err := zboxutil.NewHTTPRequest(http.MethodPost, fabricConfig.URL, fabricData)
	if err != nil {
		return "", zchainErrors.New("fabric_commit_error", "Unable to create new http request with error "+err.Error())
	}
	...
	err = zboxutil.HttpDo(ctx, cncl, req, func(resp *http.Response, err error) error {
		...
		return zchainErrors.New(strconv.Itoa(resp.StatusCode), "Fabric commit status not OK!")
	})
...
func GetAllocationMinLock(datashards, parityshards int, size, expiry int64,
	...
	if err != nil {
		return 0, zchainErrors.New("allocation_min_lock_fetch_error", "Error fetching the allocation min lock."+err.Error())
	}

	var response = make(map[string]int64)
	err = json.Unmarshal(allocationsBytes, &response)
	if err != nil {
		return 0, zchainErrors.New("allocation_min_lock_decode_error", "Error decoding the response."+err.Error())
	}
