...
// Start provides a mock function with given fields: status, isFinal
func (_m *Downloader) Start(status sdk.StatusCallback, isFinal bool) error {
	ret := _m.Called(status, isFinal)

	var r0 error
	if rf, ok := ret.Get(0).(func(sdk.StatusCallback, bool) error); ok {
		r0 = rf(status, isFinal)
	} else {
		r0 = ret.Error(0)
	}
