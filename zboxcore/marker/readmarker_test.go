...
func TestReadMarker_GetHash(t *testing.T) {
	...
	clientPublicKey := "3e7fa2dd6b924adfdf69e36cc61cb5d9012226dac619250ce5fc37eae25a05118008944d1727221b6d14b0998c5813acd13066040976598c5366e86519377001"
	...
	rm := &ReadMarker{
		...
		OwnerID:         ownerID,
		Timestamp:       common.Timestamp(timestamp.Unix()),
		ReadCounter:     readCounter,
		Signature:       "",
	}
	expectedHash := encryption.Hash(fmt.Sprintf("%v:%v:%v:%v:%v:%v:%v", allocID,
		blobberID, clientID, clientPublicKey, ownerID,
		readCounter, timestamp.Unix()))

	assert.Equal(t, expectedHash, rm.GetHash())
}
...
func TestReadMarker_Sign(t *testing.T) {
	...
	expectedSignature, err := client.Sign(encryption.Hash(fmt.Sprintf("%v:%v:%v:%v:%v:%v:%v", allocID,
		blobberID, clientID, clientPublicKey, ownerID,
		readCounter, timestamp.Unix())))
	assert.NoErrorf(t, err, "unexpected error but got %v", err)
	err = rm.Sign()
	assert.NoErrorf(t, err, "unexpected rm.Sign() error but got %v", err)
	assert.Equal(t, expectedSignature, rm.Signature)
}
