...
func Thumbnail(buf *C.char, width, height C.int, options *C.char) *C.char {
	res, err := imageutil.Thumbnail([]byte(C.GoString(buf)), int(width), int(height), C.GoString(options))
	if err != nil {
		return WithJSON("", err)
	}
	return WithJSON(res, nil)
}